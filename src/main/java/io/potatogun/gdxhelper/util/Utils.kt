 ...
 * @return 변환된 문자열
 */
inline fun parseSeconds(seconds: Int): String = parseSeconds(seconds, "'", "\"");
 ...
 * @param separator     분과 초 사이의 구분자
 * @return 변환된 문자열
 */
@JvmOverloads inline fun parseSeconds(seconds: Int, minutesSuffix: String, secondsSuffix: String, separator: String = " "): String {
	if(seconds < 60) return "${seconds}$secondsSuffix";
	return "${seconds / 60}$minutesSuffix$separator${seconds % 60}$secondsSuffix";
}
 ...
 */
@JvmOverloads fun drawText(batch: SpriteBatch, font: BitmapFont, text: String, x: Float, y: Float, color: Color = Color.WHITE, scale: Float = 1f, width: Float = 0f, align: Int = Align.left) {
 ...
 */
inline fun <T> sortWith(array: GdxArray<T>, comparator: Comparator<T>) {
